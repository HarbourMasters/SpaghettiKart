...
void render_palm_trees(Camera *camera, Mat4 arg1, UNUSED struct Actor *actor) {
    ...
    while (var_s1->pos[0] != END_OF_SPAWN_DATA) {
        ...
        if (is_within_render_distance(camera->pos, spD4, camera->rot[1], 0.0f, gCameraZoom[camera - camera1], var_f22) < 0.0f && CVarGetInteger("gNoculling", 0) == 0) {
            var_s1++;
            continue;
        }
...
void func_8029AC18(Camera *camera, Mat4 arg1, struct Actor *arg2) {
    if (is_within_render_distance(camera->pos, arg2->pos, camera->rot[1], 0, gCameraZoom[camera - camera1], 4000000.0f) < 0 && CVarGetInteger("gNoculling", 0) == 0) { return; }
